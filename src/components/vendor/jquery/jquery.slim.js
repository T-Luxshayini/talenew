		...
		"className": function( className ) {
			...
			return pattern ||
				...
				classNameCache( className, function( elem ) {
					return pattern.test( typeof elem.className === "string" && elem.className || typeof elem.getAttribute !== "undefined" && elem.getAttribute("className") || "" );
				});
		},
...
jQuery.fn.extend( {
	...
	hasclassName: function( selector ) {
		...
		while ( ( elem = this[ i++ ] ) ) {
			if ( elem.nodeType === 1 &&
				( " " + stripAndCollapse( getclassName( elem ) ) + " " ).indexOf( className ) > -1 ) {
					return true;
			}
		}
